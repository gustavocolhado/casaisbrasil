...
if (NODE_ENV === 'production') {
  ...
  try {
    options = {
      key: fs.readFileSync('/www/wwwroot/sites/confissoesdecorno/ssl/privkey.pem'),
      cert: fs.readFileSync('/www/wwwroot/sites/confissoesdecorno/ssl/fullchain.pem'),
    };
    server = https.createServer(options);
    console.log('Servidor HTTPS configurado para produção');
  } catch (error) {
    console.error('Erro ao carregar certificados SSL:', error);
    console.log('Falling back para HTTP');
    server = http.createServer();
  }
} else {
  // Configuração para desenvolvimento com HTTP
  server = http.createServer();
  console.log('Servidor HTTP configurado para desenvolvimento');
}

// Configuração do CORS baseada no ambiente
const corsOrigins = NODE_ENV === 'production' 
  ? ['https://confissoesdecorno.com', 'https://socket.confissoesdecorno.com']
  : [
      'http://localhost:3000', 
      'http://127.0.0.1:3000',
