...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 API /api/posts - Iniciando requisição')
    
    const { searchParams } = new URL(request.url)
    const limit = parseInt(searchParams.get('limit') || '10')
    const offset = parseInt(searchParams.get('offset') || '0')
    
    console.log('🔍 API /api/posts - Parâmetros:', { limit, offset })

    console.log('🔍 API /api/posts - Executando query no banco...')
    ...
    console.log('🔍 API /api/posts - Posts encontrados:', posts.length)
    console.log('🔍 API /api/posts - Primeiro post:', posts[0]?.id || 'Nenhum post encontrado')
    ...
    console.log('🔍 API /api/posts - Retornando posts transformados:', transformedPosts.length)
