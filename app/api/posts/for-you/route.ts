...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 API /api/posts/for-you - Iniciando requisição')
    
    const { searchParams } = new URL(request.url)
    const limit = parseInt(searchParams.get('limit') || '10')
    const offset = parseInt(searchParams.get('offset') || '0')
    
    console.log('🔍 API /api/posts/for-you - Parâmetros:', { limit, offset })

    // Buscar posts de todos os usuários exceto o usuário específico
    const posts = await prismaClient.post.findMany({
      where: {
        userId: { not: '686e8a5bd1907109df416f12' },
        failed: false
      },
      take: limit,
      skip: offset,
    ...
    console.log('🔍 API /api/posts/for-you - Posts encontrados:', posts.length)
    ...
    console.log('🔍 API /api/posts/for-you - Retornando posts transformados:', transformedPosts.length)
